...
public class VersionContext implements VersionInterface {
        ...
        return x -> {
          ...
          ContentSummaryComputationContext cscc =
              new ContentSummaryComputationContext(
                  fsd, fsd.getFSNamesystem(), fsd.getContentCountLimit());
          return x.computeContentSummary(cscc).getCounts().getStoragespace();
        };
      case "dirSubTreeNumFiles":
        return x -> {
          ...
          ContentSummaryComputationContext cscc =
              new ContentSummaryComputationContext(
                  fsd, fsd.getFSNamesystem(), fsd.getContentCountLimit());
          return x.computeContentSummary(cscc).getCounts().getFileCount();
        };
      case "dirSubTreeNumDirs":
        return x -> {
          ...
          ContentSummaryComputationContext cscc =
              new ContentSummaryComputationContext(
                  fsd, fsd.getFSNamesystem(), fsd.getContentCountLimit());
          return x.computeContentSummary(cscc).getCounts().getDirectoryCount();
        };
      case "storageType":
  ...
  public Map<String, Long> storageTypeHistogramCpu(
      Collection<INode> inodes, String sum, NameNodeLoader loader) {
    ...
    return loader.binMappingHistogram(
        inodes,
        sum,
        loader.getSumFunctionForINode(sum),
  ...
  public Map<String, Long> storageTypeHistogramCpuWithFind(
      Collection<INode> inodes, String find, NameNodeLoader loader) {
    List<Long> distinctStorageIds = StorageTypeHistogram.bins;
    List<String> distinctStorageKeys = StorageTypeHistogram.keys;
    Map<String, Long> storageIdToIndexToKeyMap =
        distinctStorageIds
    ...
    return loader.binMappingHistogramWithFind(
        inodes,
        findOp,
        loader.getFilterFunctionToLongForINode(findField),
        node -> (long) distinctStorageIds.indexOf((long) node.getStoragePolicyID()),
        storageIdToIndexToKeyMap);
  }
